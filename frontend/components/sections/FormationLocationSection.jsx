...
export default function FormationLocationSection() {
  const t = useTranslations('FormationLocationSection');
    ...
    <section className="bg-white px-6 py-20 md:py-28">
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2 md:items-center">
        ...
        >
          ...
          <p className="mt-6 flex items-start gap-3 font-inter text-sm text-dark">
            <MapPin size={18} className="mt-0.5 shrink-0 text-gold-500" />
            {t('address')}
          </p>
          <p className="mt-3 flex items-start gap-3 font-inter text-sm text-dark">
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: '-50px' }}
        >
          <MapEmbed title={t('mapTitle')} src={MAPS_EMBED_SRC} height="350" />
        </motion.div>
      </div>
    </section>
  );
